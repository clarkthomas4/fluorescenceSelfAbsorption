...
def tomography(nxsfileName,dataFolder, centre, index):
    ...
    if not contLoop:
        if index==0:
            ...
            #filtered=tomopy.remove_stripe_fw(npdata, level=20, wname='db5', sigma=1, pad=1)
            #norm=tomopy.normalize_roi(npdata, roi=[0,80,9,85])
            #filtered=tomopy.remove_stripe_fw(norm, level=0, wname='haar', sigma=0.1, pad=0)
        ...
        rec = tomopy.recon(pippo, theta, mycent, algorithm='mlem', num_iter=40)#'osem''ospml_hybrid', num_iter=100)#algorithm='gridrec')
        #recRingless=tomopy.remove_ring(rec,43,43,rwidth=1,theta_min=20,thresh=1,thresh_max=0.4, thresh_min=-0.4)
...
if __name__ == "__main__":
    ...
    pathToNexustomoData='/dls/i13-1/data/2017/cm16785-1/processing/VortexTomo/vortexProjectionsPtAttenuation0103.hdf'
    centre=23
    #width=
